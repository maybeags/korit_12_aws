...
- 현재 server port를 80으로 잡아놨는데, nginx가 80을 점유합니다. 그래서 springboot의 server port를 8080으로 수정해야 할 것 같습니다.
- 수정하세요.

- 근데 방금 서버를 켜서 도메인을 통해서 white label이 뜨는지를 체크했으니까 다시 서버를 꺼야 합니다. 

- 그리고 우리가 고려해야할 사항이
![alt text](image-9.png)
라는 점입니다.
보안 규칙으로 80으로 잡혀있다. 그런데 application.yml을 8080으로 수정했으니 이 부분이 문제가 되는 것 아니냐. -> 정답
...
server {
  listen 80;
  server_name api.maybeags-springboot.p-e.kr;
...
그 결과 `nginx: configuration file /etc/nginx/nginx.conf test is successful`라고 떴다면 성공입니다.
...
### 사용 이유
1. 리버스 프록시 : 보안을 위해 사용자가 실제 애플리케이션 서버(저희 기준으로는 8080)에 직접 접근하지 못하게 막고, 지가 받아가지고 토스해주는 역할을 함.
2. 로드밸런싱 : ELB라는 것은 AWS에서의 서비스 명이고 load balancing 개념 자체는 다른 데서도 쓰입니다. 접속자가 너무 많을 때 여러 대의 서버로 요청을 분산시켜서 서버 다운을 방지하고, 서버 다운 되면 다른 데로 보내주는 등의 역할을 합니다.
3. 정적 파일 처리 : 기본적으로 static 폴더 내에 있는 HTML / CSS / 이미지 등에 있는 변하지 않는 정적 파일들을 애플리케이션 서버를 거치지 않고 직접 응답시켜줘서 효율을 높일 때 사용.(resources/static이라고 건드린 적이 있습니다 -> 근데 우리는 react 써서 상관없음)
...
sudo certbot --nginx -d api.maybeags-springboot.p-e.kr
```
...
2. sudo ln -s /snap/bin/certbot /usr/bin/certbot : 두 개의 경로 적힌 것에서 볼 수 있듯이 실행 파일을 /usr/bin/으로 바로가기 만드는 겁니다.
  - 터미널 어느 링크든지간에 상관없이 certbot 명령어 실행시킬 수 있도록 바로가기 만드는겁니다.

3. sudo certbot --nginx -d api.maybeags-springboot.p-e.kr : sudo 권한으로 certbot 실행시키는데, `--nginx`를 통해서 nginx 설정 파일을 **알아서 HTTPS로 수정해라** 는 의미. `-d 도메인주소`는 인증서 받을 도메인 주소를 지정합니다(저희는 현재 서브도메인에 탄력적 ip 연결해놨기 때문에 이렇게 했습니다). -> 그러면 프론트엔드는요? 프론트엔드는 정적 페이지 배포를 통해서 별개의 도메인 발급받습니다.

근데 이거 실행 한 줄 씩 하고나면 뭐 이것저것 물어봅니다.

1. Email address 질문이 있습니다 : 이메일 입력하고 엔터쳤습니다.
2. Terms of Service : 약관같은겁니다. 동의하는지 물어보는 거.
3. 광고 볼거니 : N 했습니다.

- rebuild 시도했었는데 생각해보니까 안해도 됩니다. 저희는 springboot 프로젝트를 수정한게 아니라 EC2 내부에 있는 nginx 파일만 수정한거니까요.

그래서 certificate가 통과되면 https://api.여러분도메인 으로 접속이 가능해야 합니다.

이제 뭐 해야 하냐면 -> 다 지워야 합니다. 돈 나오기 싫으면.


- 내일 새로운 방식으로 springboot 프로젝트를 실행할 예정입니다. 차이점은 터미널을 꺼도 springboot 프로젝트가 켜져 있는지 아닌지의 여부입니다. 현재 sudo로 실행할 경우에는 터미널 끄면 프로젝트도 같이 꺼집니다.
...
git add .
git commit -m "feat: 20260324 EC2 / ELB / Nginx / HTTPS"
git push